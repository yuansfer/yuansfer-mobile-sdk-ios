...
class WechatPayViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // 注册微信
        WechatPay.registerWechat(appid: HttpUtils.APP_ID, universalLink: HttpUtils.UNIVERSAL_LINK)
    }
    ...
    private func submitPayment(_ result: [String: Any]) {
        let appid = result["appid"] as! String
        let partnerid = result["partnerid"] as! String
        let prepayid = result["prepayid"] as! String
        let package = result["package"] as! String
        let noncestr = result["noncestr"] as! String
        let timestamp = result["timestamp"] as! String
        let sign = result["sign"] as! String
        
        if !WechatPay.isAppInstalled() {
            self.resultLabel.text = "Wechat not installed"
            return
        }
        
        if !WechatPay.isWXAppSupportApi() {
            self.resultLabel.text = "Wechat not support"
            return
        }
